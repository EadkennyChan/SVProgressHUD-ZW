...
@property (nonatomic, assign)BOOL bEnable;//开关
...
static SVProgressHUD *g_sharedHUDView;

+ (SVProgressHUD*)sharedView
{
    static dispatch_once_t once;
    dispatch_once(&once, ^ {g_sharedHUDView = [[self alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
        g_sharedHUDView.bEnable = YES;});
    if (g_sharedHUDView.bEnable)
        return g_sharedHUDView;
    else
        return nil;
}
...
+ (void)HUDEnable:(BOOL)bEnable
{
    g_sharedHUDView.bEnable = bEnable;
}
...
+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType superview:(UIView *)superview
{
    [[self sharedView] showProgress:-1 status:status maskType:maskType superview:superview];
}

- (void)showProgress:(float)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType superview:(UIView *)superview
{
    if(!self.overlayView.superview)
    {
        if (superview == nil)
        ...
        {
            self.overlayView.center = superview.center;
            [superview addSubview:self.overlayView];
            [superview bringSubviewToFront:self.overlayView];
        }
    }
